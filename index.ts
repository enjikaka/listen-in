      ...
      eventTarget.addEventListener("scrobble", (e) => {
        if (e instanceof CustomEvent) {
          if (e.detail.checksum !== lastEventId) {
            controller.enqueue(
              createEvent("scrobble", e.detail, e.detail.checksum),
            );
          }
        }
      });
...
async function handleRequest(request: Request): Promise<Response> {
  ...
  if (!user) {
    return new Response("User not specified", { status: 400 });
  }

  try {
    return await userFeed(request, user);
  } catch (e) {
